...
func _process(delta):
	...
	if limit_left <= margin: limit_left = -margin + 1;
	if limit_right >= margin: limit_right = margin - 1;
	if limit_top <= margin: limit_top = -margin + 1;
	if limit_bottom >= margin: limit_bottom = margin - 1;
